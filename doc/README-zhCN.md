...
- 可将生成文件名中的重音字符和连字转换为 ASCII 等价形式，提高文件系统及外部工具的兼容性。
...
| 自动检索导入 PDF 元数据  | 自动识别无父条目的导入 PDF；仅在 Zotero 创建父条目后执行重命名/移动。               |
...
| 移除文件名重音符号       | 转换最终文件基本名（包括保留的原文件名前缀）中的重音字符和连字，并保持扩展名不变。    |
